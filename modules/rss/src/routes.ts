/**
 * Express routes for the RSS module.
...
router.get('/data', async (req: Request, res: Response) => {
  const instanceId =
    typeof req.query.instanceId === 'string' ? req.query.instanceId : 'rss_01';
  ...
  if (!config.feedUrl) {
    return res.json({
      ...
      message: 'No feed URL configured. Set feedUrl in module config.',
    });
  }
  ...
  try {
    ...
    return res.json({ instanceId, data: feedData });
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Unknown error';
    console.error('[routes] /data error for instance %s:', instanceId, err);
    return res.status(502).json({ error: 'Failed to fetch feed data', detail: message });
  }
});
...
router.post('/action', async (req: Request, res: Response) => {
  ...
  const id = typeof instanceId === 'string' ? instanceId : 'rss_01';
