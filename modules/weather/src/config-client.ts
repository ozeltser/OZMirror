/**
 * Config Service client for the Weather module.
 ...
 */
export async function fetchInstanceConfig(instanceId: string): Promise<WeatherConfig> {
  if (!INSTANCE_ID_PATTERN.test(instanceId)) {
    console.warn(`[config-client] Invalid instanceId rejected: ${instanceId}`);
    return DEFAULT_CONFIG;
  }
  try {
    const { data } = await axios.get<WeatherConfig>(
