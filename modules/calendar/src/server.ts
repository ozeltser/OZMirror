/**
 * OzMirror Calendar Module — Express server entry point.
 *
 * Lifecycle:
 *  1. Start Express server
 *  2. Connect to Redis (publisher + cache client)
 *  3. Register with Config Service (retries built-in)
 *  4. Start periodic calendar refresh (every 15 minutes)
...
const server = app.listen(PORT, HOST, async () => {
  console.log(`[server] Calendar module listening on ${HOST}:${PORT}`);
